template: title

.mega-octicon.octicon-circuit-board[]

# Greatest Hits from Ask-Git-Core

Patrick McKenna

---

template: content

# The What .octicon.octicon-circuit-board[]

A selection of:

  - how to's

  - general advice for working with Git

---

template: content

# Workshop mechanics .octicon.octicon-circuit-board[]

Assumptions:

- You're comfortable with command line Git

- You understand the basics of Git's internals

- You're familiar with the basics of shell scripting

---

template: content

# Workshop mechanics .octicon.octicon-circuit-board[]

Assumptions:

- You're comfortable with command line Git

- You understand the basics of Git's internals

- You're familiar with the basics of shell scripting

Note: examples checked on Git 2.11.0

---

template: section

.mega-octicon.octicon-circuit-board[]

# Simplifying common tasks

---

template: content

# Simplifying common tasks .octicon.octicon-circuit-board[]

For example: 

What commits were added to `<branch>` by my last `pull`?

--

```bash
git-show-new <branch>
```

---

template: content

# List commits added to `<branch>` by last `pull` .octicon.octicon-circuit-board[]

First, we need accept a `<branch>` arg

--

... and provide a sensible default if none supplied

--

```bash
#!/bin/bash

if [ $# -ge 1 ]; then
  branch="$1"
else
  branch="HEAD"
fi
# ...
```

---

template: content

# List commits added to `<branch>` by last `pull` .octicon.octicon-circuit-board[]

Next: generate the list of new commits

--

Use `git rev-list` (it generates and traverses commit graphs)


---

template: content

# List commits added to `<branch>` by last `pull` .octicon.octicon-circuit-board[]

General syntax for commits reachable from `A` but not `B`:

--

```bash
git rev-list A ^B
```

--

(`A`, `B` are [commit-ish](https://www.kernel.org/pub/software/scm/git/docs/gitglossary.html))

--

Could use alternate sytax (but readability an issue)

```bash
git rev-list B..A
```

---

template: content

# List commits added to `<branch>` by last `pull` .octicon.octicon-circuit-board[]

```bash
git rev-list A ^B
```

--

In our case:

- `A` is the commit where `<branch>` currently points

- `B` is the commit `<branch>` pointed to before our latest `pull`

---

template: content

# List commits added to `<branch>` by last `pull` .octicon.octicon-circuit-board[]

How do we specify the commit `<branch>` used to point to?

--

Using the `@{}` syntax:

```bash
<branch>@{1}
```

---

template: content

# List commits added to `<branch>` by last `pull` .octicon.octicon-circuit-board[]

Hence:

```bash
git rev-list <branch> ^<branch>@{1}
```

---

template: content

# List commits added to `<branch>` by last `pull` .octicon.octicon-circuit-board[]

A final version might look like this:

```bash
#!/bin/bash

if [ $# -ge 1 ]; then
  branch="$1"
else
  echo "git-show-new requires a branch name!"
  exit 1
fi

printf "\n%s%s\n\n" $(git rev-list $branch ^$branch@{1} | wc -l) \
  " commits were added by your last update to $branch:"
git --no-pager log $branch ^$branch@{1} --oneline
```

---

template: content

# Related advice .octicon.octicon-circuit-board[]

Avoid Git syntax shortcuts in scripts

--

```bash
BASE=$(git merge-base @ @{u} 2>/dev/null || echo "none")
```

--

An engineer wondered: "what are those `@` symbols?!"

--

*Those are the Unicode code points for the "the Git command line was getting too comprehensible" character in Swahili*

---

template: content

# Avoid Git syntax shortcuts in scripts .octicon.octicon-circuit-board[]

```bash
BASE=$(git merge-base @ @{u} 2>/dev/null || echo "none")
```

--

A character to introduce special syntax:
  
- the `@` is short for `HEAD`

- `@{u}` is short for `@{upstream}`

--

*Using that in scripting is a terrible idea, IMHO.*

---

template: content

# Avoid Git syntax shortcuts in scripts .octicon.octicon-circuit-board[]

Git can be tricky enough as is 

--

Make scripts reader-friendly where possible

--

```diff
-BASE=$(git merge-base @ @{u} 2>/dev/null || echo "none")
+BASE=$(git merge-base HEAD @{upstream} 2>/dev/null || echo "none")
```

---

template: content

# Get by with a little help from your Git-friends .octicon.octicon-circuit-board[]

--

Git includes `git-sh-setup.sh` "scriplet"

--

- meant for inclusion in other scripts

- offers utility functions

- manages a few potentially problematic env vars

---

template: section

.mega-octicon.octicon-circuit-board[]

# Integrating external tooling with Git

---

template: content

# Integrating external tooling with Git .octicon.octicon-circuit-board[]

- add branch metadata

- using exclude patterns in scripts

---

template: content

# Add metadata to a branch .octicon.octicon-circuit-board[]

--

How:

```bash
git branch --edit-description <branch>
```

--

data written to `.git/config`

--

- follows renames
- deleted when branch deleted

---

template: content

# Add metadata to a branch .octicon.octicon-circuit-board[]

`.git/config` meant to be extensible for third party uses

--

Side note about branches:

*Git doesn't have any notion of "this commit was made on this branch"*


---

template: content

# Using  in scripts .octicon.octicon-circuit-board[]

--

Ex: testing scripts that respect contents of:

- `.gitignore`
- `.git/info/exclude`

---

template: content

# Filter based on Git exclude patterns .octicon.octicon-circuit-board[]

```bash
git check-ignore
```

--

Notes:

--

- need list of files, or list of dirs (including all subdirs)
  - no recursive option for wildcard searches
- can't check list against custom ignore file
  - closely coupled to Git's usage of ignore files
- not listed in docs landing page `https://git-scm.com/docs` (but it's there)


---

template: content

# Filter based on Git exclude patterns .octicon.octicon-circuit-board[]

What if you don't have a list of files already?

--

```bash
# show untracked files in output
git ls-files --others
```

---

template: content

# Filter based on Git exclude patterns .octicon.octicon-circuit-board[]

Ex: pre-commit hook that for Python linting

--

```bash
pep8 \
  --exclude $(git ls-files --others $python_dirs | tr '\n', ',') \
  $python_dirs
```

---

template: section

.mega-octicon.octicon-circuit-board[]

# Moving history between repos

---

template: content

# Moving history - scenario .octicon.octicon-circuit-board[]

2 separate repos, each with history

--

Import `<subdir>` from `<source_repo>` into `<target_repo>`

--

(and preserve history of `<subdir>`)

---

template: content

# Moving history - scenario .octicon.octicon-circuit-board[]

Notes:

--

- a one-time operation
  - e.g. split apart large project
  - submodules inappropriate
- assume `<subdir>` does not exist in `<target_repo>`, and that we want to use the same name

---

template: content

# Moving history  .octicon.octicon-circuit-board[]

Step 1: construct a "synthetic" history

--

(all commits that touch files in `<subdir>`)

--

In `<source_repo>`:

```bash
git subtree split --branch <branch> --prefix <subdir>
```

---

template: content

# Moving history .octicon.octicon-circuit-board[]

```bash
git subtree split --branch <branch> --prefix <subdir>
```

---

template: content

# Moving history .octicon.octicon-circuit-board[]

```bash
git subtree split --branch <branch> --prefix <subdir>
```

- creates a new branch

---

template: content

# Moving history .octicon.octicon-circuit-board[]

```bash
git subtree split --branch <branch> --prefix <subdir>
```

- creates a new branch
- synthesizes commit history

---

template: content

# Moving history .octicon.octicon-circuit-board[]

```bash
git subtree split --branch <branch> --prefix <subdir>
```

- creates a new branch
- synthesizes commit history
  - each commit has contents of `<subdir>` at root of project
  - repeated splits produce identical synthetic histories

---

template: content

# Moving history .octicon.octicon-circuit-board[]

Step 2: get contents of `<subdir>` into `<target_repo>`:

--

```bash
git subtree add --prefix <subdir> <source-repo> <branch>
```

---

template: content

# Moving history .octicon.octicon-circuit-board[]

```bash
git subtree add --prefix <subdir> <source-repo> <branch>
```

--

- imports synthetic history, puts contents into `<subdir>`

---

template: content

# Moving history - caveats .octicon.octicon-circuit-board[]

`git log` may not work as you expect

-- 

```bash
>>> git log -- <path>
~/projects/bash_utils/s1 (master) > git log -- <path>
commit 2e259f35fea13ca6bbfb8bd33be6476caea5cc86
Merge: e6a4851 d0e7410
# ...
    Add '<subdir>' from commit 'd0e74107e4517e48888b3de5bd7b129bc12a041f'
    
    git-subtree-dir: <subdir>
    git-subtree-mainline: e6a4851ca13c9fa76fd25277b1d1bb6cd7e61519
    git-subtree-split: d0e74107e4517e48888b3de5bd7b129bc12a041f
```

---

template: content

# Moving history - caveats .octicon.octicon-circuit-board[]

`git log` may not work as you expect

--

(and adding `--follow` doesn't help)

--

Why just one commit touching `<path>`?

--

Can work around this:

--

```bash
git log -- *<path>
```

---

template: content

# Moving history - v2 .octicon.octicon-circuit-board[]

In `<target-repo>`

--

```bash
git checkout -b <branch>
git remote add  -f <remote-name> <source-repo>
git merge --allow-unrelated-histories <remote-name>/<branch>
```

---

template: content

# Moving history - general notes .octicon.octicon-circuit-board[]

- `filter-branch` generalizes better (to processing all branches), but `subtree` simpler
- former ships with Git, while `subtree` lives in `contrib`
- but `filter-branch` is "notoriously tricky to use" (!)
- on GitHub: "Commits that don’t share a common merge base with the base branch are not supported"

---

template: content

# Collaborate on merging .octicon.octicon-circuit-board[]

[git-imerge](https://github.com/mhagger/git-imerge)

---


- can't use built-in commands as an alias
- "should we expand aliases for commands" an early debate, a case of Git not wanting to let people hurt themselves!

- `git checkout --orphan` to create new branch w/o parents, disconnected from all history
- for an abbreviated history, see filter branch with grafts

- https://submitgit.herokuapp.com/

- `git add -p <untracked-file>`
  - optionally preceded by `git add -N`

